  ...
  return (
    <Box>
      ...
      <Paper elevation={0} sx={{ p: 2, mb: 3 }}>
        <Typography variant="h6" sx={{ mb: 2 }}>偏好设置</Typography>
        <List disablePadding>
          <ListItemButton>
            <ListItemText primary="显示成人内容" secondary="显示评分较高的成人内容" />
            <Switch edge="end" checked={nsfwEnabled} onChange={handleNsfwToggle} />
          </ListItemButton>
          <ListItemButton>
            <ListItemText primary="仅显示成人内容" secondary="只显示成人内容，隐藏其他番剧" />
            <Switch edge="end" checked={nsfwOnly} onChange={handleNsfwOnlyToggle} />
          </ListItemButton>
          <Divider sx={{ my: 1 }} />
          <ListItemButton>
            <ListItemText primary="动态取色" secondary="根据系统强调色动态调整主题" />
            <Chip label="已启用" color="primary" variant="outlined" />
          </ListItemButton>
          <Box sx={{ p: 2 }}>
            <Typography variant="body2" sx={{ mb: 1 }}>自定义主题色（MD3 风格）</Typography>
      ...
      <Paper elevation={0} sx={{ p: 2 }}>
        <Typography variant="h6" sx={{ mb: 2 }}>数据管理</Typography>
        <List disablePadding>
        ...
        <DialogContent dividers>
          <Box sx={{ whiteSpace: 'pre-wrap', fontFamily: 'monospace', maxHeight: '500px', overflowY: 'auto' }}>
            {logs.length > 0 ? (
                ...
                <Typography variant="body2">暂无日志记录</Typography>
            )}
          </Box>
        </DialogContent>
        <DialogActions>

...
// import HistoryIcon from '@mui/icons-material/History'; // 你的代码中没有使用
// import ArticleIcon from '@mui/icons-material/Article'; // 你的代码中没有使用
...
function AppContent() {
  ...
  const handleLogoutClick = () => {
    setUserInfo(null);
    window.showSnackbar('info', '已成功退出登录');
  };
  ...
  const theme = createTheme({
    ...
    components: {
      ...
      MuiListItemButton: {
        styleOverrides: {
          // 使用函数形式，避免在 createTheme 阶段就引用 theme 自身
          root: ({ theme }) => ({
            borderRadius: 12,
            margin: '8px 16px',
            '&.Mui-selected': {
  ...
  // isMobile 必须在 ThemeProvider 内部，才能正确获取 theme
